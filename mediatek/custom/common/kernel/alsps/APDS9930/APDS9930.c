			...
			else
			{
				...
				if(obj->als == 0)
				{
					...
					*/
					...
					/* light sensor debounce filter. oujf1 2012-03-13 */
					APDS9930_read_als(obj->client, &obj->als);
					//APS_LOG("get sensor data %d old als %d D-value = %d\n", obj->als, old_als, abs(obj->als - old_als));

					if (abs(obj->als - old_als) < MAX_BOUND)
					{
						obj->als = old_als;
					}
