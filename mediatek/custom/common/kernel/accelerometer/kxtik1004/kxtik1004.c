...
static int KXTIK1004_SetDataResolution(struct kxtik1004_i2c_data *obj)
{
	...
	{
		GSE_ERR("kxtik1004 read Dataformat failt \n");
...
static int KXTIK1004_CheckDeviceID(struct i2c_client *client)
{
	...
	if(false)
	{
		GSE_ERR("KXTIK1004_CheckDeviceID 0x%x failt!\n ", databuf[0]);
		return KXTIK1004_ERR_IDENTIFICATION;
	}
	else
	{
		GSE_ERR("KXTIK1004_CheckDeviceID 0x%x pass!\n ", databuf[0]);
	}
...
static int KXTIK1004_SetDataFormat(struct i2c_client *client, u8 dataformat)
{
	...
	GSE_ERR("KXTIK1004_SetDataFormat OK! \n");
...
static int KXTIK1004_SetBWRate(struct i2c_client *client, u8 bwrate)
{
	...
	{
		GSE_ERR("kxtik1004 read rate failt \n");
		return KXTIK1004_ERR_I2C;
	}
	...
	GSE_ERR("KXTIK1004_SetBWRate OK! \n");
...
static int kxtik1004_init_client(struct i2c_client *client, int reset_cali)
{
	...
	GSE_ERR("kxtik1004_init_client OK!\n");
#ifdef CONFIG_KXTIK1004_LOWPASS
...
static int KXTIK1004_ReadSensorData(struct i2c_client *client, char *buf, int bufsize)
{
    ...
    /*
     * chenyong5, add error detection when reading data. 2013-05-08
     * this will fix the wrong data and screen can't rotate issue when
     * rebooting tablet repeatly.
     * check CTRL_REG1
     */
    read_count ++;
    res = hwmsen_read_byte(client, KXTIK1004_REG_POWER_CTL, &data);
    if (res) {
        GSE_ERR("Read CTRL_REG1(0x1B) error %d!\n", res);
    } else {
        //check PC1 bit and GSEL1, GSEL0 bit
        if (((data & 0x80)  == 0)|| ((data & 0x18) != 0)) {
            GSE_ERR("[%d-%d]CTRL_REG1 is not right, value=%x\n", read_count, error_count++, data );
            // set the CTRL_REG1 to right value
            data = data & (~0x18);
            GSE_ERR("Write CTRL_REG1 to value %x\n", data);
            data &= ~KXTIK1004_MEASURE_MODE;
            GSE_ERR("Write CTRL_REG1 to value %x\n", data);
            res = hwmsen_write_byte(client, KXTIK1004_REG_POWER_CTL, data);
            if (res) {
                GSE_ERR("Write CTRL_REG1 to right value error %d!\n", res);
            }
            // let the SetPowerMode to set the system to operating mode;
            sensor_power = false;
            GSE_ERR("SetPowerMode to true, change chip to operating mode\n");
...
//enable selftest bit
	KXTIK1004_SetPowerMode(client,false);// chenyong5
...
static ssize_t store_register_value(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	GSE_ERR("databuf[0]=0x%2x  databuf[1]=0x%2x \n", databuf[0],databuf[1]);
...
static ssize_t show_register_value(struct device_driver *ddri, char *buf)
{
		...
		GSE_ERR("i2c_dev_reg=0x%2x  data=0x%2x \n", i2c_dev_reg,databuf[0]);
...
static ssize_t show_debug_value(struct device_driver *ddri, char *buf)
{
    ...
    for (i =0x1B; i<=0x21;i++) {
        ...
        len += snprintf(buf + len, PAGE_SIZE - len, "REG:%x = %x\n", i, data);
...
{
    GSE_ERR("dummy function\n");
    return 0;
}

static DRIVER_ATTR(debug,      S_IWUSR | S_IRUGO, show_debug_value,         store_debug_value);
/*----------------------------------------------------------------------------*/
