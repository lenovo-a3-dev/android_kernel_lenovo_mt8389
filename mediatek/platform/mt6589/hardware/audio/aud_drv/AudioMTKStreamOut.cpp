...
#ifndef ENABLE_AUDIO_DRC_SPEAKER
        mAudioResourceManager->doSetMode();
#else
	audio_mode_t Mode = mAudioResourceManager->GetAudioMode();
	mAudioResourceManager->doSetMode();
	if(Mode != AUDIO_MODE_IN_COMMUNICATION && mAudioResourceManager->GetAudioMode() == AUDIO_MODE_IN_COMMUNICATION)
	{
	    SetStreamOutCompFltStatus(AUDIO_COMP_FLT_AUDIO, false);
...
status_t AudioMTKStreamOut::SetStreamOutCompFltStatus(AudioCompFltType_t eCompFltType, bool bEnable)
{
    ...
    if (false == mStreamOutCompFltEnable[eCompFltType]) {
        if (true == bEnable) {
#ifdef ENABLE_AUDIO_DRC_SPEAKER //VOIP Skip DRC 
	    AudioComFltMode_t eFltMode; 
	    if(AUDIO_COMP_FLT_AUDIO == eCompFltType && mAudioResourceManager->GetAudioMode() == AUDIO_MODE_IN_COMMUNICATION) 
	    { 
#ifdef ENABLE_AUDIO_COMPENSATION_FILTER 
		eFltMode = AUDIO_CMP_FLT_LOUDNESS_COMP; 
#else 
		eFltMode = AUDIO_CMP_FLT_LOUDNESS_NONE; 
#endif 
	    } 
	    else 
	    { 
		eFltMode = mdCompFltMode[eCompFltType]; 
	    } 
	    mpClsCompFltObj[eCompFltType]->SetWorkMode(channels()==AUDIO_CHANNEL_OUT_MONO?1:2, sampleRate(), eFltMode); 
#else 
